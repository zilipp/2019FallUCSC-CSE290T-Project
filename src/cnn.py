...
class CNN:
    ...
    def __init__(self, feature_size):
        ...
        x = layers.Conv1D(100, kernel_size=3, activation='relu')(x)
        x = layers.Conv1D(100, kernel_size=3, activation='relu')(x)
...
if __name__ == "__main__":
    ...
    X_train = np.array(data_train['title'].to_list())
    T_train = np.array(data_train['label'])
    X_test = np.array(data_test['title'].to_list())
